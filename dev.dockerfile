...
RUN bash -lc 'eval "$(mise env -s bash)"; \
  ...
  uv pip install cairosvg csvkit dprint yt-dlp docling httpx pandas pillow ruff llm typer rich orjson lxml tenacity pytest typer google_genai "playwright==${PLAYWRIGHT_VERSION}"; \
...
# Takes ~1.5 min
RUN bash -lc 'eval "$(mise env -s bash)"; \
  echo "04 Sep 2026: Updating agents and fast-moving agent tools"; \
