...
def _event_kind(raw: dict[str, Any]) -> str:
    ...
    if not isinstance(raw_type, str) or not raw_type:
        return "unknown"
    if raw_type in {"user", "assistant", "system"}:
        return raw_type
    if raw_type in {"session.start", "session_meta"}:
        return "session"
    if raw_type == "turn_context":
        return "context"
    if raw_type in {"tool.execution_start", "tool.execution_complete"}:
        return "tool"
    if raw_type == "event_msg":
        ...
        if isinstance(payload_type, str) and payload_type:
            return _normalize_kind(payload_type)
        return "event"
    if raw_type == "response_item":
        payload = raw.get("payload")
        payload_type = payload.get("type") if isinstance(payload, dict) else None
        if payload_type == "message" and isinstance(payload, dict):
        ...
        return "response"
    if "." in raw_type:
        return _normalize_kind(raw_type.split(".", 1)[0])
    return _normalize_kind(raw_type)


def _matches_kind(event: SessionEvent, kind_filter: frozenset[str]) -> bool:
    return not kind_filter or _event_kind(event.raw) in kind_filter


def _parse_search_pattern(search: str) -> SearchPattern | None:
    if not search:
        return None
    if search.startswith("/"):
        closing = search.rfind("/")
        if closing <= 0:
            raise re.error("missing closing '/' in regex search")
        flags_text = search[closing + 1 :]
        if set(flags_text) - {"i"}:
            raise re.error(f"unsupported regex flags: {flags_text}")
        flags = re.IGNORECASE if "i" in flags_text else 0
        return SearchPattern(pattern=search, regex=re.compile(search[1:closing], flags=flags))
    return SearchPattern(pattern=search, needle=search.casefold())


def _search_text_matches(text: str, search_re: SearchPattern | None) -> bool:
    return search_re is None or search_re.search(text)


def _search_value_matches(value: Any, search_re: SearchPattern | None) -> bool:
    if search_re is None:
        return True
    if isinstance(value, str):
        return _search_text_matches(value, search_re)
    if isinstance(value, dict):
        return any(_search_value_matches(item, search_re) for item in value.values())
    if isinstance(value, list):
        return any(_search_value_matches(item, search_re) for item in value)
    if value is None:
        return False
    return _search_text_matches(str(value), search_re)


def _search_event_matches(raw: dict[str, Any], search_re: SearchPattern | None) -> bool:
    return _search_value_matches(raw, search_re)
...
def _truncate_search_line(text: str, width: int) -> str:
    if width <= 0:
        return ""
    if len(text) <= width:
        return text
...
def _search_preview_width(width: int) -> int:
    return max(1, width - 4)


def _extract_matching_lines(value: Any, search_re: SearchPattern | None, width: int) -> list[str]:
    if search_re is None:
        return []
    if isinstance(value, str):
        matches: list[str] = []
        for line in value.splitlines() or [value]:
            if _search_text_matches(line, search_re):
                matches.append(_truncate_search_line(line, _search_preview_width(width)))
        return matches
    if isinstance(value, dict):
        matches: list[str] = []
        for item in value.values():
            matches.extend(_extract_matching_lines(item, search_re, width))
        return matches
    if isinstance(value, list):
        matches: list[str] = []
        for item in value:
    ...
    if value is None:
        return []
    return _extract_matching_lines(str(value), search_re, width)


def _append_search_preview(summary: SessionSummary, lines: list[str]) -> None:
    if not lines:
        return
    ...
    summary.search_preview = "\n".join(preview_lines)


def _can_stream_ls(
    *, cwd_filter: str, since: str, until: str, search_re: SearchPattern | None
) -> bool:
    del search_re
...
def _print_summary(summary: SessionSummary, colors: Colors) -> None:
    ...
    body = summary.search_preview or summary.first_prompt
    ...
    print(colors.wrap(colors.prompt, indent_block(body).rstrip()), flush=True)
    print(
...
class ClaudeBackend:
    ...
    def list_sessions(self, **kwargs: Any) -> tuple[list[SessionSummary], Stats]:
        ...
        search_re: SearchPattern | None = kwargs["search_re"]
        stats = Stats()
        ...
        for session in sessions.values():
            if cwd_filter and cwd_filter not in session.cwd:
                continue
            if search_re is not None and not getattr(session, "_search_matched", False):
                continue
            if not include_empty and not (session.search_preview or session.first_prompt):
                continue
    ...
    def _db_matches_search(
        ...
        *,
        ...
        if search_re is None:
            return []
        lines: list[str] = []
        for text in (cwd, summary, first_prompt):
            lines.extend(_extract_matching_lines(text, search_re, width))
        for row in conn.execute(
            "select user_message, assistant_response from turns where session_id = ?",
            (session_id,),
        ):
            lines.extend(_extract_matching_lines(row["user_message"] or "", search_re, width))
            lines.extend(_extract_matching_lines(row["assistant_response"] or "", search_re, width))
        deduped: list[str] = []
        seen: set[str] = set()
        for line in lines:
            if line in seen:
    ...
    def list_sessions(self, **kwargs: Any) -> tuple[list[SessionSummary], Stats]:
            ...
            for row in conn.execute(
                ...
                if first_prompt:
                    ...
                    if (not allow_local_commands) and looks_like_local_command(
                        ...
                        first_prompt = ""
                search_preview = self._db_matches_search(
                    conn,
                    sid,
                    cwd=cwd,
                ...
                if search_re is not None and not search_preview:
                    continue
                if not include_empty and not (search_preview or first_prompt):
                    continue
                summaries.append(
                    SessionSummary(
                        session_id=sid,
        ...
        for path in self._legacy_files(root, stats):
            ...
            if not matched_search:
                continue
            if not include_empty and not (summary.search_preview or summary.first_prompt):
                continue
            ...
            for path in self._legacy_files(root, stats):
                ...
                for _, event in rows:
                    ...
                    if not matched_search and _search_event_matches(event, search_re):
                        matched_search = True
                    _append_search_preview(
                        summary, _extract_matching_lines(event, search_re, width)
                    )
                    _append_search_preview(
                        summary, _extract_matching_lines(event, search_re, width)
                    )
...
def build_app(backend: Backend) -> typer.Typer:
    ...
    def list_sessions(
        ...
        width: int = typer.Option(
            80, help="Truncate displayed search-match lines to this total width."
        ),
        ...
        search: str = typer.Option(
            "",
            help="Search session content. Use /pattern/ or /pattern/i for regex; plain text is a fast case-insensitive substring search.",
        ),
    ) -> None:
        colors = Colors()
        try:
            search_re = _parse_search_pattern(search)
        except re.error as exc:
            raise typer.BadParameter(f"Invalid regex: {exc}") from exc
        args = dict(
        ...
        if _can_stream_ls(cwd_filter=cwd, since=since, until=until, search_re=search_re):
    ...
    @app.command()
    def md(
        session_ids: list[str] = typer.Argument(..., help="Session IDs to render."),
        root: Path = typer.Option(
            backend.default_root, help=f"Log root (default: {backend.default_root})."
        ),
        strict: bool = typer.Option(False, help="Fail fast on invalid JSON lines."),
        ...
        for session_id in session_ids:
            ...
            _merge_stats(total_stats, event_stats)
            if not events:
                if stats:
                    print(json_pretty(dataclasses.asdict(total_stats)), file=sys.stderr)
                raise typer.Exit(code=3)
            markdowns.append(
                backend.render_markdown(
                    session_id=session_id,
                    events=events,
                    include_meta=include_meta,
                    open_details=open_details,
                    kind_filter=_parse_kind_filters(kind),
                ).rstrip()
            )
        markdown = "\n\n".join(part for part in markdowns if part)
        if out == "-" or not out:
